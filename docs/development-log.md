...
三路结果暂不直接相加，因为 Milvus 相似度和网页搜索顺序不是同一种分数。下一步先使用基于名次的 RRF 融合直接检索与 HyDE 两路本地片段；网页结果保留到重排阶段再与本地候选比较。
...
第 16 步将实现 RRF 本地多路融合：按直接向量和 HyDE 结果各自的名次计算融合分数，以 `chunk_id` 去重后形成本地候选列表；网页结果继续独立保留到重排阶段。
...
还原课程 day12 的 RRF（Reciprocal Rank Fusion，倒数排名融合）节点：读取直接向量检索和 HyDE 检索的有序结果，不比较两路原始相似度，而是根据各片段在每一路中的名次计算统一分数，以 `chunk_id` 去重、排序并截取候选。

网页结果本步不进入 RRF。课程在后续 Reranker 节点中先合并“RRF 本地片段 + 网页文档”，再使用同一个重排器比较相关性；这样不会为了让没有 `chunk_id` 的网页摘要参与本地去重而人为制造不可靠的键。

### 与上一步的目录区别

第 15 步已经产生三路独立结果。本步新增目录项如下：
...
| 新文件 | 作用 |
| --- | --- |
| `backend/app/workflows/querying/nodes/rrf.py` | 按来源权重和名次计算 RRF 分数，在同一路内先去重，再跨路线累加、稳定排序并生成统一本地候选。 |
| `backend/tests/test_rrf_node.py` | 验证两路共同命中提升、权重、最大数量、稳定同分顺序、单路重复保护、无效记录过滤和零权重路线。 |

### 本步修改的已有文件

| 文件 | 修改内容 |
| --- | --- |
| `.env.example` | 增加 RRF 平滑参数、最大结果数和直接/HyDE 两路权重。 |
| `backend/README.md` | 补充 RRF 配置、公式、响应字段和网页结果暂不参与的原因。 |
| `backend/app/core/config.py` | 增加 RRF 参数范围与“两路权重不能同时为零”的组合校验。 |
| `backend/app/workflows/querying/state.py` | 增加带 `rrf_score` 和 `source_paths` 的融合片段类型及 `rrf_results` 状态。 |
| `backend/app/workflows/querying/graph.py` | 让直接检索与 HyDE 节点共同汇入 RRF 节点，网页分支继续独立结束。 |
| `backend/app/workflows/querying/nodes/__init__.py` | 导出 `RrfNode`。 |
| `backend/app/schemas/queries.py` | 响应新增 `fused_matches`，返回 RRF 分数、命中路线和安全片段字段。 |
| `backend/tests/test_config.py` | 验证 RRF 默认参数和全零权重拒绝规则。 |
| `backend/tests/test_query_workflow.py` | 验证 LangGraph 汇合点读取到两路完整结果，并在三路并行节点之后执行 RRF。 |
| `backend/tests/test_query_api.py` | 验证 `fused_matches` 的 RRF 分数和来源数组能安全返回。 |
...
```text
RRF(d) = Σ weight(route) / (k + rank(route, d))
```

- `rank` 从 1 开始，只看片段在该路线中的名次。
- `weight` 控制路线影响力，本项目直接检索和 HyDE 默认都是 `1.0`。
- `k` 是平滑参数，默认 `60`；它让第一名与后几名的差距保持温和，避免某一路第一名完全压过另一条路线的共同命中。
- 片段没有出现在某一路时，该路线不贡献分数。

示例：片段 A 在直接检索排第 1、HyDE 排第 2：
...
因此 A 通常排在 B 前面：两种问法都找到的片段比只被一路找到的片段更稳定。
...
课程基础代码直接遍历列表；本项目额外处理了同一路重复片段，避免一个异常重复结果在同一路被加分多次。文档正文保留首次出现的版本，来源遍历顺序为直接检索优先、HyDE 其次，因此共同命中时优先保留真实问题直接召回的片段数据。

片段 ID 为整数 `0` 时仍视为合法值，避免使用 `if not chunk_id` 把零值误删；布尔值、负数、非字典记录和同一路重复 ID 会被忽略。RRF 分数相同时使用首次出现顺序作为稳定的第二排序条件，保证相同输入产生相同输出。
...
query_embedding_node
       ├─ vector_search_node ─┐
       ├─ hyde_search_node ───┴─→ rrf_node ────────────→ END
       └─ web_search_node（独立保留）───────────────────→ END
```

LangGraph 只有在直接检索和 HyDE 两个上游节点都完成后才执行 `rrf_node`。网页节点不依赖 RRF；如果网页开关关闭，它快速返回 `disabled`，如果开启则可与本地检索并行执行。
...
`matches`、`hyde_matches` 和 `web_matches` 暂时继续返回，便于开发阶段观察每一路原始结果；下一步重排与答案生成稳定后，可以再评估是否缩减生产响应。`fused_matches` 不沿用 Milvus 的 `score` 字段，而使用含义明确的 `rrf_score`，防止把原始相似度和融合分混淆。
...
两路权重可以分别设为 `0` 来关闭某一路对融合分数的影响，但不能同时为 `0`。RRF 只进行字典累计和列表排序，不调用模型、数据库或外部 API，几乎不增加服务器常驻资源；候选最多 10 条也为下一步重排控制了输入规模。

### 测试与验证

- Ruff 静态检查与格式检查通过。
- Pytest 共 232 个测试通过，5 个显式集成测试默认跳过。
- RRF 单元测试不访问 Token、网络、Milvus、MongoDB 或本地模型。
- 工作流测试验证了 RRF 汇合点同时取得直接和 HyDE 结果，而不是在其中一路完成后提前运行。
- 完整回归继续覆盖前 15 步全部功能。

### 当前边界

- RRF 依据名次而不是内容本身判断相关性；共同命中通常更可靠，但不代表内容一定正确。
- 默认 `k=60`、权重 `1.0/1.0` 和最大 10 条是课程开发初值，后续应使用真实问答集评估。
- 网页摘要仍未与本地片段统一比较，也没有网页正文抓取。
- 当前没有 `knowledge_chunks` 集合，因此没有执行真实本地两路融合查询。

### 下一步

第 17 步将还原 Reranker 精排：合并 RRF 本地候选和网页结果，使用适合当前 2 核 2 GiB 服务器的云端重排 API 重新计算“问题—文档”相关性，并实现断崖检测动态截取。
